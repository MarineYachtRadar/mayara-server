...
//! Integration test: replay Navico Broadband 3G pcap fixture.
//!
...
#[tokio::test]
async fn replay_navico_3g() {
    let fixture = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("testdata")
        .join("pcap")
        .join("navico-3g.pcap.gz");
    if !fixture.exists() {
        panic!(
                ...
                loop {
                    ...
                    if !keys.is_empty() {
                        ...
                        if info.controls.model_name().is_some() && !info.ranges.all.is_empty() {
                            ...
                            assert_eq!(info.controls.model_name().unwrap(), "3G");
                            assert!(!info.doppler, "3G should not support Doppler");
