...
if ! command -v git-cliff &>/dev/null; then
    echo "Error: git-cliff not found. Install from https://git-cliff.org/ (brew install git-cliff)"
    exit 1
fi
...
# Rewrite CHANGELOG.md for the release being cut.
#
# git-cliff groups commits by tag, and the tag does not exist yet when the
# release PR is built, so --tag tells it which version the unreleased commits
# belong to. The hand-written history of everything before this scheme lives in
# CHANGELOG.manual.md and is appended verbatim.
generate_changelog() {
    local tag="$1"
    git-cliff --config cliff.toml --tag "$tag" --output CHANGELOG.md
    printf '\n' >>CHANGELOG.md
    cat CHANGELOG.manual.md >>CHANGELOG.md
    echo "Regenerated CHANGELOG.md for ${tag}"
}
...
Commands:
  --release     Release the current version:
                  ...
                  • regenerates CHANGELOG.md with git-cliff, in the PR
